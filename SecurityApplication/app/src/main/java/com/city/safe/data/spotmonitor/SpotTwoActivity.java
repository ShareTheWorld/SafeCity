 ...
 *
 */
public class SpotTwoActivity extends Activity implements CvCameraViewListener2, OnClickListener {
	...
	public class MyHandler extends Handler{
		...
		public void handleMessage(Message msg) {
			...
			String str="";
			if(msg.arg1!=-10000){
				str="Counts："+imagenum+" Coordinates："+"x(2-1)"+(msg.arg1)*rate+" y(2-1)"+(msg.arg2)*rate;
				result.setText(str);
	...
	public void createFile(){
		...
		fs=new FileSaver("two_spot_monitor_data"+".txt");
	}
	...
	public Mat onCameraFrame(CvCameraViewFrame inputFrame){
		...
		if(workingMode  == GlobalData.ModeDarkDetection){
			...
			for(int ii = upperLefty ; ii<clipWidth ; ii++){
				whiteLine = true;
				for(int jj = upperLeftx; jj < clipHeight; jj++){
					color = pixels[width * jj + ii];
				...
				if(findone && whiteLine){
					break;
				}
			}
			boolean findtwo = false;
			boolean whiteLine2 = true;
			for(int ii = upperLefty + clipWidth; ii > upperLefty ; ii--){
				whiteLine2 = true;
				for(int jj = upperLeftx; jj < clipHeight; jj++){
					color = pixels[width * jj + ii];
					red = ((color & 0x00FF0000) >> 16);
				...
				if(findtwo && whiteLine2){
					break;
				}
			}
			Message msg = new Message();
			if( countLeft > 0 && countRight > 0){
				int leftx = sumLeftx/countLeft;
				int lefty = sumLefty/countLeft;
				int rightx = sumRightx/countRight;
				int righty = sumRighty/countRight;
				int deltaxUsed = righty - lefty;
				int deltayUsed = leftx - rightx;
				++imagenum;
				msg.arg1 = deltaxUsed;
				msg.arg2 = deltayUsed;
