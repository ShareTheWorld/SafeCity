...
public class ImageProvider {
	...
	public Bitmap fetchBitmapData(ImageLoader imageLoader, ImageTask imageTask,
		...
		if (DEBUG) {
			...
			Log.d(TAG, String.format(MSG_FETCH_BEGIN_IDENTITY_KEY, imageTask,
			...
			Log.d(TAG, String.format(MSG_FETCH_BEGIN_IDENTITY_URL, imageTask,
		...
		if (inputStream == null) {
			if (reuseInfo != null && reuseInfo.getReuseSizeList() != null
				...
				for (int i = 0; i < sizeKeyList.length; i++) {
					...
					if (inputStream != null) {
						if (DEBUG) {
							Log.d(TAG, String.format(MSG_FETCH_REUSE_SUCCESS,
									imageTask, size));
						}
						break;
					} else {
		...
		if (imageTask.getStatistics() != null) {
			imageTask.getStatistics().s2_afterCheckFileCache(
					inputStream != null);
		}
		...
		if (inputStream == null) {
			...
			inputStream = mDiskCacheProvider.downloadAndGetInputStream(
					imageLoader.getImageDownloader(), imageTask, fileCacheKey,
					url);
			if (imageTask.getStatistics() != null) {
			...
			if (inputStream == null) {
				...
				CLog.e(TAG, "%s download fail: %s %s", imageTask, fileCacheKey,
						url);
			}
		}
		if (inputStream != null) {
			try {
				bitmap = decodeSampledBitmapFromDescriptor(inputStream.getFD(),
						imageTask, imageReSizer);
				if (bitmap == null) {
					imageTask.setError(ImageTask.ERROR_BAD_FORMAT);
					CLog.e(TAG, "%s decode bitmap fail, bad format. %s, %s",
		...
		if (imageTask != null && imageTask.getStatistics() != null) {
			imageTask.getStatistics().s4_afterDecode(
					mDiskCacheProvider.getSize(fileCacheKey));
		}
		return bitmap;
	}
