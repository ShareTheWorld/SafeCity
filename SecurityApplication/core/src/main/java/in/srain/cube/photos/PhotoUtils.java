...
public class PhotoUtils {
	...
	public static void toCamera(Activity activity, File outputFile,
			int requestCode) {
		if (!isCameraUseAble(activity)) {
			Toast.makeText(activity, R.string.cube_photo_no_camera,
		...
		if (resolveInfoList == null && resolveInfoList.size() == 0) {
			Toast.makeText(activity, R.string.cube_photo_can_not_use_camera,
					Toast.LENGTH_LONG).show();
			return;
		}
	...
	public static void toAlbum(Activity activity, int requestCode) {
		try {
			...
			Toast toast = Toast.makeText(activity,
					R.string.cube_photo_can_not_open_album, Toast.LENGTH_SHORT);
			toast.setGravity(Gravity.CENTER, 0, 0);
			toast.show();
		}
	}

	public static void toCrop(Activity activity, File file, File outputFile,
			CropOption info, int requestCode) {
	...
	// columns, idWhere, new String[]{id}, null);
	// } else {
	// cursor = activity.getContentResolver().query(uri, columns, null, null,
	// null);
	// }
	// if (cursor != null) {
	// int column_index =
