...
public final class SelectPhotoManager {
	...
	public void start(final Activity activity, final SelectClickHandler handler) {
		...
		DiskFileUtils.CacheDirInfo info = DiskFileUtils.getDiskCacheDir(
				activity, TEMP_PATH_NAME, 1024 * 1024 * 30);
		File path = info.path;
		try {
		...
		mTempDir = info.path;
		mActivity = activity;
		mTempFile = new File(mTempDir.getAbsolutePath(), Long.toString(System
				.nanoTime()) + ".jpg");
		...
		DialogInterface.OnClickListener clickListener = new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int i) {
				...
				case ACTION_TAKE_PHOTO:
					PhotoUtils.toCamera(activity, mTempFile,
							REQUEST_CODE_CAMERA);
					break;
				case ACTION_ALBUM:
	...
	private boolean afterPhotoTaken(String imgPath) {
		...
		mTempFile = new File(mTempDir.getAbsolutePath(), Long.toString(System
				.nanoTime()) + "_cropped.jpg");
		PhotoUtils.toCrop(mActivity, f, mTempFile, mCropOption,
				REQUEST_CODE_CROP);
		return true;
	}
}
